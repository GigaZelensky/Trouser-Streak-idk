            ...
            for (int i = 0; i < 199; i++) {
                ...
                if (mc.world.getBlockState(isopenair1).isReplaceable()
                    ...
                    if (player.hasVehicle()) {
                        ...
                        for (int pn = 0; pn < packetsRequired - 1; pn++) {
        ...
        builder.then(literal("down").executes(ctx -> {
            ...
            for (int i = 0; i > -199; i--) {
                ...
                if (mc.world.getBlockState(isopenair1).isReplaceable()
                    ...
                    player.fallDistance = 0;  // prevent fall damage when landing
                    mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.OnGroundOnly(true, mc.player.horizontalCollision));
                    return SINGLE_SUCCESS;
                }
            }
        ...
        builder.then(literal("highest").executes(ctx -> {
            ...
            for (int i = 199; i > 0; i--) {
                ...
                if (!mc.world.getBlockState(isopenair1).isReplaceable()
                    ...
                    player.fallDistance = 0;  // ensure no damage from highest teleport
                    mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.OnGroundOnly(true, mc.player.horizontalCollision));
                    return SINGLE_SUCCESS;
                }
            }
